...
- **Telegram gateway setup/troubleshooting**: See `references/gateway-telegram-setup-notes.md` for practical notes on BotFather tokens, numeric Telegram user IDs, allowlists, foreground gateway runs when services are unsupported, transient `httpx.ReadError` reconnects, and fixing ownership problems under `HERMES_HOME`.
