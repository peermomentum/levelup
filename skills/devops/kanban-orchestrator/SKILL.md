...
## Worker Execution and Handoff Standards

The former `kanban-worker` skill is now a worker-facing section of this umbrella because orchestrator and worker behavior are one Kanban class-level workflow. Workers should:

- Start by reading task state and comments; stop if the task is blocked, archived, or reassigned.
- Respect `$HERMES_KANBAN_WORKSPACE` kind (`scratch`, `dir:<path>`, or `worktree`) and avoid writing outside it unless the card explicitly permits it.
- Use precise heartbeats for long work only; skip noisy `still working` updates.
- Use comments for durable context and `kanban_block(reason=...)` for decisions requiring human input.
- Use `kanban_complete(summary=..., metadata=...)` only for genuinely terminal work; code-changing tasks that need review should block with a `review-required:` reason after posting changed files, tests, and diff/PR evidence as a comment.
- Capture returned IDs from successful `kanban_create` calls before claiming `created_cards`; never invent card IDs from prose.
- Prefix persistent memory with `$HERMES_TENANT` when tenant isolation is active.

Good handoff metadata includes changed files, tests run/pass/fail, decisions made, PR/diff paths, sources read, and approval status. A snapshot of the former worker-specific package is stored at `references/absorbed-kanban-worker/`.
