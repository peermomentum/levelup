...
- Concierge must not submit member-requested buddies from first-name-only, last-name-only, nickname, or otherwise partial names (for example `Ryan`). It must use `buddy_concierge_pairing` action `resolve_requested_buddy`; if the result is `clarification_required`, ask the member to confirm the full roster name (e.g. “Did you mean Ryan Brown?”). Only submit admin review with the canonical full `requested_buddy` and `requested_buddy_record_id`, because the email watcher requires linked roster records after admin approval.
- If an approved request failed member-email sending because the admin approved a shorthand/ambiguous requested-buddy name (for example `Ryan` instead of `Ryan Brown`) and `Requested Buddy Record` / `Selected Buddy` is missing, resolve the person against Team Roster first, then PATCH the request with the canonical `Requested Buddy` name plus both linked-record fields, clear `Member Email Error`, set `Member Email Status = Needs Sending`, run the email watcher in `--dry-run --verbose`, then run it live and verify Gmail SENT message IDs plus Airtable `Member Email Status = Sent`. Do not manually compose replacement emails unless the watcher cannot process the request after the Airtable link repair.
